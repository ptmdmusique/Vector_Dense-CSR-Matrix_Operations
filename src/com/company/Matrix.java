...
public class Matrix {
    ...
    public LinkedList<Matrix> LUFactorization(){
        ...
        for(int indx = 0; indx < size; indx++){
            upperMatrix.GetMatrix()[0].SetEntry(indx, matrix[0].GetEntry(indx));
        }

        //The first column of L is firstColA / A[0][0]
        for(int indx = 0; indx < size; indx++){
            lowerMatrix.GetMatrix()[indx].SetEntry(0,
                    matrix[indx].GetEntry(0) / matrix[0].GetEntry(0));
        }
        ...
        for(int row = 1; row < size; row++){
            ...
            for(int col = row; col < size; col++){
                ...
                if(upperMatrix.GetEntry(row, col) == 0){
